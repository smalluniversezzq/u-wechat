  ...
  <div id="goods" ref="goods" v-if="other">
    ...
    <div id="box" ref="box">
      <div class="goods_carousel" ref="swiper">
        ...
        <!--</swiper>-->
        <swiper :options="swiperOption" ref="smallImages" v-else>
      ...
      <div class="goodsInst">
        ...
        <div class="update" v-if="other.level_commission_money && (other.level_commission_money !== other.commission_money)" @click="jumpTo('/rightsConter')">
          <p :style="{color: $store.state.global.theme.mainColor}">现在升级{{$up_operator}}，最高获得{{other.level_commission_money}}佣金</p><span :style="{background: 'linear-gradient('+$store.state.global.theme.deg+'deg,' +$store.state.global.theme.mainColor+','+$store.state.global.theme.secondColor+')'}">立即升级</span>
        </div>
        <div class="coupon" @click="tobuy" v-if="other.coupon_money">
          <img src="../../assets/img/coupon/coupon.png" alt="" v-if="$isCouponPass === '1'">
          <img src="../../assets/img/home/goods/couponBg.png" alt="" v-else>
      ...
      <!--<template v-if="other && other.rate && other.rate.rateList">-->
        <!--<div class="commentContent"  v-for="(item, index) of other.rate.rateList" :key="index">-->
          ...
          <!--<div class="imgbox" >-->
            <!--<div  v-for="(img, i) of item.images" :key="i" :style="{backgroundImage: 'url(http:' + img + ')'}"></div>-->
          <!--</div>-->
        <!--</div>-->
      <!--</template>-->
      <!--<div class="shopBoxInfo">-->
        <!--<img class="shopLogo" src="../../assets/img/home/pddshop.png" alt="" v-if="type !== 2 && type === 3">-->
        <!--<img class="shopLogo" v-else-if="type !== 2 && other.shopIcon" v-lazy="other.shopIcon" alt="" :key="other.shopIcon">-->
        <!--<img class="shopLogo" v-else-if="type !== 2 && Number(other.type) === 11" src="../../assets/img/home/tbshop.png" alt="">-->
        <!--<img class="shopLogo" v-else-if="type !== 2 && Number(other.type) === 12" src="../../assets/img/home/tmshop.png" alt="">-->
        <!--<div class="shopInfo" v-if="other.shopName">-->
          <!--<div class="title">-->
            <!--<p><img src="../../assets/img/super/tb1.png" alt="" v-if="type === 0"><img src="../../assets/img/super/tm.png" alt="" v-else-if="type === 1"><img src="../../assets/img/super/jd.png" alt="" v-else-if="type === 2"><img src="../../assets/img/super/pdd.png" alt="" v-else-if="type === 3">{{other.shopName}}</p>-->
            <!--<span v-if="other.shopUrl && !$route.query.from && type !== 2" @click="joinShop(other.shopUrl, other.shopName)" :style="{background: 'linear-gradient('+$store.state.global.theme.deg+'deg,' +$store.state.global.theme.mainColor+','+$store.state.global.theme.secondColor+')'}">进店逛逛</span>-->
            <!--<span v-else-if="other.shopUrl && !$route.query.from && type === 2" @click="joinJd(other.shopUrl)" :style="{background: 'linear-gradient('+$store.state.global.theme.deg+'deg,' +$store.state.global.theme.mainColor+','+$store.state.global.theme.secondColor+')'}">进店逛逛</span>-->
          <!--</div>-->
          <!--<div class="score" v-if="type !== 2 && other.score">-->
    ...
    <div class="footer">
      ...
      <div class="collect" @click="share">
        <img src="../../assets/img/share.png" />
        <p>分享赚￥{{other.commission_money}}</p>
      </div>
      <div class="otherBox" @click="tobuy">
        <div class="Box"><p>券后￥{{other.coupon_price}}</p><p>再省￥{{other.commission_money}}</p></div>
        <div class="Box"><p>领券购买</p></div>
      </div>
    </div>
    <div class="goodsBarrage" :class="{showBarrage: showBarrage}" v-if="barrage">
    ...
    <div class="shade" v-if="isShowPop">
      <div class="popUp">
        <div class="word">淘口令购买
          <span class="mistake" @click="isShowPop = !isShowPop">×</span>
        </div>
      <div class="copy">
        <span>长按选择&gt;&gt; </span>
        <span class="copys">拷贝</span>
      </div>
      <div id="code" class="code">
        <p>{{other.title}}</p>
        <p>原价：{{other.zk_final_price}}元</p>
        <p>优惠券：{{other.coupon_money}}元</p>
        <p>券后价：{{other.coupon_price}}元</p>
        <span class="line"></span>
        <p>复制这条信息打开【手机淘宝】领券下单</p>
        <span >{{other.tbk_pwd}}</span>
        <input type="text" v-model="msg" id="foo">
      </div>
      <div id="duplication" class="duplication" data-clipboard-target="#foo"  @click="copy">复制</div>
        <span class="sm">点击复制后，请打开【手机淘宝】购买!</span>
    </div>
  </div>
</div>
</template>
...
<script>
import {XImg, Previewer, TransferDom, dateFormat, XDialog, Confirm} from 'vux'
...
export default {
  ...
  methods: {
    ...
    copy () {
      this.msg = this.other.title + '\n【在售价】' + this.other.zk_final_price + '元\n【券后价】' + this.other.coupon_price + '元\n---------' + '复製这条' + this.other.tbk_pwd + ',进入【Tao宝】即可抢购'
      var clipboard = new Clipboard('.duplication')
    ...
    lookRate () {
      if (window.api) {
        ...
        this.$vue.toast('请在app中打开')
      }
    },
    joinShop (url, shopName) {
      let that = this
      api.openWin({
        name: 'aliWin',
        url: 'widget://html/main.html',
    ...
    goodsBack () {
      ...
      } else if (this.$route.query.from === 'jd') {
        utils.storage.set('supergoods', [{}])
        this.$router.go(-1)
        api.openWin({
          name: 'jdWin',
      ...
      } else if (window.api && api.pageParam.backToWin) {
    ...
    collect () {
      ...
      if (this.other.coupon_url) {
        this.$http.post('/amoy/user/collection', {
          ...
          commission_money: this.other.commission_money,
          level_commission_money: this.other.level_commission_money,
    ...
    oauth () {
      let that = this
      window.location.href = '#/oauth?url=' + encodeURIComponent(that.other.oauth_url)
    },
    tobuy () {
      if (!this.$store.state.user.token) {
        this.$router.push('/wechatLogin')
      ...
      if (this.$isCouponPass === '1' && this.other.coupon_money > this.couponpassMoney) {
      ...
      if (that.other.coupon_url) {
        if (this.type === 0 || this.type === 1) {
          if (this.other.rid === '' && this.other.oauth_url !== '') {
            ...
            //     aliBC.showLogin(function (ret, err) {
            //       if (ret.status) {
            //         if (that.$isCouponPass === '1') {
            //           that.$http.post('/amoy/couponpass/reduce-money', {
            //             type: that.$route.query.type,
            //             img: that.other.small_images[0],
            //             title: that.other.title,
            //             coupon_money: that.other.coupon_money
            //           }, true, true).then(res => {
            //             if (res.code === 0) {
            //               that.couponpassMoney = res.data.couponpass_money
            ...
            if (that.$isCouponPass === '1') {
              that.$http.post('/amoy/couponpass/reduce-money', {
                ...
                if (res.code === 0) {
                  that.couponpassMoney = res.data.couponpass_money
                  that.$store.commit('setCouponpassMoney', res.data.couponpass_money)
                  window.open(that.other.coupon_url)
          ...
          //           if (response.code === 0) {
          //             that.couponpassMoney = response.data.couponpass_money
          //             that.$store.commit('setCouponpassMoney', response.data.couponpass_money)
          //             let browser = api.require('webBrowser')
          //             browser.open({
          //               url: that.other.coupon_url,
          //               titleBar: {
    ...
    getJD () {
      ...
      this.$http.post('/amoy/jd/shop-name', {
        item_id: this.$route.query.id
      }, true).then(res => {
        if (res.code === 0) {
          this.$set(this.other, 'shopName', res.data.shop)
          this.$set(this.other, 'shopUrl', res.data.url)
          this.local(this.other)
        }
      })
      if (!this.$store.state.user.token) {
        ...
        if (this.$route.query.way || !this.other.title) {
          this.$http.post('/amoy/jd/search', {
            ...
            for (let i in res.data[0]) {
              this.$set(this.other, i, res.data[0][i])
            }
    ...
    getBarrage () {
      this.$http.post('/amoy/taobao/barrage', {
        id: this.$route.query.id
      }, false, true).then(res => {
        if (res.code === 0) {
          this.getBarrage = res.data
          this.barrage = res.data[0]
  ...
  mounted () {
    ...
    document.getElementsByClassName('goods_back_top_box')[0].style.bottom = '1.4rem'
    let that = this
    if (window.api) {
...
<style lang="less" scoped>
  ...
  .word{
    ...
    text-align: center;
    line-height: 1rem;
    background: -webkit-gradient(linear, right top, left top, from(#ff8e76), to(#ff5847));
    background: linear-gradient(270deg, #ff8e76, #ff5847);
    color: #fff;
  }
  .mistake{
    position: absolute;
