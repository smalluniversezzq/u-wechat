    ...
    <div v-transfer-dom>
      <popup v-model="show1">
        <div class="refund-foot">
          ...
          <div class="list" v-for="(item,i) in modelInfo" @click="handChange1(i,item)">
            <p>{{item.title}}</p>
            <div class="checkbox">
...
export default {
  ...
  methods: {
    ...
    applyConfirm () {
      ...
      apiHttp.applyConfirm('save', this.applyInfo.shopName, this.cid, this.model, this.applyInfo.userName, this.applyInfo.userPhone, this.addressAll[0], this.addressAll[1], this.addressAll[2], this.address, this.phone).then(res => {
        if (res.code === 0) {
