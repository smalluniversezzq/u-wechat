<template>
  <div class="turnOut">
    <p class="title">您当前持有的{{data.credit_alias ? data.credit_alias : '集分宝'}}</p>
    ...
    <p class="exchange" :style="{color: $store.state.global.theme.mainColor}">注：{{data.exchange_ratio}}个{{data.credit_alias ? data.credit_alias : '集分宝'}}可以兑换一个区块币</p>
  ...
  methods: {
    sure () {
      ...
      if (Number(this.num) > Number(this.data.credit4)) {
        this.$vux.toast.text('您当前所持有的' + this.data.credit_alias + '数量不足')
        return
      }
