...
export default {
  ...
  methods: {
    ...
    upCallback (page, mescroll) {
      var that = this
      // 联网请求
      this.$http.get('/amoy/share-video/video-search', {keyword:that.search,type:that.type+1,page:page.num - 1}, true).then((res) => {
  ...
  mounted () {
    var that = this
    this.search = decodeURIComponent(that.$route.query.value)
  }
}
</script>
