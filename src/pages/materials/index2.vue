...
export default {
  ...
  components: {
    Tab, TabItem, MescrollVue, barrage, swiper, swiperSlide
  },
  ...
  methods: {
    ...
    changeUrl (i, type) {
      if (this.item.items[i].goods_id) {
        this.$http.post('/amoy/user/get-url', {
          id: this.item.items[i].goods_id,
          item_url: this.item.items[i].coupon_url,
          type: this.item.items[i].from_id
        }, true, true).then(res => {
          if (res.code === 0) {
            let img = this.link + '/amoy/user/create-share?num_iid=' + this.item.items[i].goods_id + '&item_url=' + res.data + '&type=' + this.item.items[i].from_id + '&title=' + encodeURI(this.item.items[i].title) + '&thumb=' + this.item.items[i].img_url + '&coupon_money=' + this.item.items[i].coupon_money + '&coupon_price=' + this.item.items[i].coupon_price + '&origin_price=' + this.item.items[i].zk_final_price + '&uid=' + this.$store.state.user.userInfo.uid
