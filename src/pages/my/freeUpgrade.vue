    ...
    <div class="freeUpgrade" ref="freeUpgrade" v-if="data" :style="{overflowY: dialog ? 'hidden' : 'auto'}">
      ...
      <div class="upgradeBox" ref="upgradeBox">
        <div class="head">
          ...
          <div class="powerBox" v-if="data && data.rights.length > 0">
            <div class="power">
              ...
              <div class="update" @click="update">升级{{title}}</div>
            </div>
          </div>
        </div>
        <div class="status">
          <p class="select" v-if="data && data.choiceRights.length > 0">{{title}}精选权益</p>
          ...
          <div class="select">如何升级为{{title}}</div>
        ...
        <div class="persons" v-if="data && data.managerList.length > 0">
          <p class="title" v-if="$bizId === 4">我们的运营总监</p>
          <p class="title" v-else>我们的运营商</p>
...
export default {
  ...
  methods: {
    ...
    checkWx (type) {
      if (!this.wxCheck.hasMpOpenid) {
        this.$store.commit('wechatAuthoriation', {url: '/freeUpgrade'})
      } else {
        this.getWay(type)
      }
    },
    ...
    getInfo () {
      this.$http.post('/amoy/user/manager',{ "id": this.id }, true).then(res => {
        this.data = res.data
  ...
  created () {
    ...
    } else if (this.$up_operator){
      this.title = this.$up_operator
    } else {
      this.title = '运营商'
    }
