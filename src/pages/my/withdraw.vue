...
export default {
  ...
  methods: {
    ...
    // 提现
    widthdraw () {
      // 如果用户没有绑定过微信公众号的openid，则前去授权绑定
      if (!this.wechatStatus.hasMpOpenid) {
        this.$store.commit('wechatAuthoriation', '/withdraw')
        return
      }
      if (!this.info.withdraw_to && this.way === 'alipay') {
        this.$vux.toast.text('请先绑定账户')
        return
      }
      // if (!this.info.wechat_openid && this.way === 'wxpay') {
      //   this.$vux.toast.text('请先绑定账户')
      //   return
      // }
    ...
    * */
    checkUserWechat () {
      this.$http.get('/wechat/user/wx-check', {}).then(res => {
    ...
    * */
    updateWechat () {
      this.$http.post('/wechat/user/update-wx-info', {code: this.$utils.getQuery()['code'], device: 'mp'}).then(res => {
      })
    }
  },
  mounted () {
    this.checkUserWechat()
    this.getInfo()
